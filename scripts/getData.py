...
def search_results_loop(page_num):
    ...
    if check(proxy, 3) == True:
        ...
        try:
            ...
            boxers.to_csv(r'./boxers.csv', index=False, mode='a', na_rep='NaN')
            all_bouts.to_csv(r'./all_bouts.csv', index=False, mode='a', na_rep='NaN')
...
def write_errors(lst):
    with open(r'./profile_errors.csv', 'w') as f:
        writer = csv.writer(f)
